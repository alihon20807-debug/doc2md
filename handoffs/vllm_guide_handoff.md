...
This file is the single source of truth for continuing this work. See also:
- `docs/VLLM_SESSION_JOURNAL.md` — narrative account of how each bug was
  found, in order, including the dead ends. Read that if you want the
  "why did this take so long" story; read this file for the reference facts.
- `CLAUDE.md` — the condensed version of the gotchas below, meant for quick
  orientation at the start of a session, not full detail.
...
1. **Wire doc2md's CLI to actually use this model** (see §9 — this is the
   biggest gap). `doc2md/cli.py`'s `--vllm-model` default is still
   `google/gemma-3-4b-it`, completely unrelated to what's actually running.
   Either change the default in `cli.py` or always pass
   `--vllm-model cyankiwi/gemma-4-12B-it-qat-AWQ-INT4` explicitly. Then run
   a **real** `uv run doc2md sample_docs/test.pdf -o out/` and confirm the
   output looks right — this has never been done this session. All
   throughput testing so far bypassed doc2md's own pipeline entirely.
...
- **doc2md has never actually converted a real document against this
  ...
  are all unexercised.
- **`doc2md/cli.py`'s `--vllm-model` default is still wrong** (`google/gemma-3-4b-it`,
  a completely different, much smaller model that was never the subject of
  this session's work). Nothing in this session changed that default —
  running plain `uv run doc2md ...` without `--vllm-model` explicitly set
  will NOT use the model this whole document is about.
